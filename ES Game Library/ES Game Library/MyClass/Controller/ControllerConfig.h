...
public:
	...
	bool GetButtonBuffer(int button_num) {
		return PadBuffer().IsPressed(button_num);
	}

	GamePadState PadState() { return GamePad(pad_number)->GetState(); }

	bool GetButtonState(int button_num) {
		return PadState().Buttons[button_num - GamePad_Button1];
	}
	...
	bool KeyboardButtonState(int button_num) {
		return KeyboardState().IsKeyDown(button_num);
	}

	bool KeyboardButtonBuffer(int button_num) {
		return keyboardBuffer.IsPressed(button_num);
	}

	static void GetKeyboardBuffer()
	{
		keyboardBuffer = Keyboard->GetBuffer();
	}
